...
def train_model(files_list_, device='cpu',serialized_model=False):
    ...
    output_files = './training_result/'+datetime.now().strftime('%Y%m%d_%H%M%S')+str(config.sigma_max)+'_'+str(config.sigma_min)+'_'+str(config.embed_dim)+'_'+str(config.hidden_dim)+'_'+str(config.num_encoder_blocks)+'_'+str(config.num_attn_heads)+'_output/'
...
def generate(files_list_, load_filename, device='cpu', serialized_model=False):
    ...
    nshowers_per_file = [1311,6685,774,613,615]#[6557,33427,3872,3067,3077]#
...
#                trained_model_name = 'training_20240408_1350_output/ckpt_tmp_299.pth'
                trained_model_name = '/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/training_result/training_quantile_fulldataset20250105_1014_output/ckpt_tmp_200.pth'
                output_directory = generate(files_list_, load_filename=trained_model_name, device=device)
