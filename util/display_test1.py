...
class High_class_feature_plot_test:
    def __init__(self, source_file, reference_files, output_dir):
        ...
        i = 0 
        self.batches = [2,2]#[1311,6685,774,613,615,2]
        ...
        for ref_file in reference_files:
            ...
            print(self.shower_ref_list[i].size())
    ...
    def plot_energy_r(self):
        ...
        energy_per_r_layer_gen = [self.reshaped_shower_gen[:, :, :, i].sum() / self.shower_num for i in range(self.r_bins)]
        energy_per_r_layer_ref = [self.reshaped_shower_ref[:, :, :, i].sum() / self.shower_num for i in range(self.r_bins)]
    ...
    # Other methods like plot_energy_z, r_width, max_voxel_dep_energy_layer would follow the same logic,
    # handling the different number of showers per reference file.