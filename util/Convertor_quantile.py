...
class Preprocessor:
    ...
    def transform_hit_xy(self, hit_pos):
      transformer = joblib.load("/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/quantile_modified/quantile_transformer_all.pkl")
      ...
      #joblib.dump(transformer, "./test_data/quantile_transformer_all.pkl")
      #transformer = joblib.load("/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/test_dataset/quantile_transformer.pkl")
      #new_pos = transformer.transform(hit_pos) + 4.0
      return new_pos
    ...
    def transform_hit_e(self, e_, incident_energy):
      new_e = e_ / (incident_energy * 2.+1e-10) 
    ...
    def inverse_transform_hit_e(self, e_, mask, new_padding_value, incident_energy):
      ...
      new_e = (((np.exp(e_))/(1.+(np.exp(e_)))) - 1e-6) * (2. * incident_energy + 1e-10) / (1.-2e-6)
...
class Convertor_ref:
    ...
    def load_dataset(self, dataset_name, label):
      ...
      for i, file_path in enumerate(dataset_name):
          ...
          # Append the selected data, ensuring they are tensors
          

          # Slice the data to get the required batches
          #selected_data = (data[0][:batches_to_take], data[1][:batches_to_take])
      ...
      # combined_inputs = torch.cat([d[0] for d in combined_data])
      # combined_targets = torch.cat([d[1] for d in combined_data])
      # label_tensor = torch.ones(len(combined_inputs), device=self.device) * label
