...
def train_model(files_list_, device='cpu',serialized_model=False):
    ...
    output_files = './training_result/training_epoch_1000_different_shower'+datetime.now().strftime('%Y%m%d_%H%M%S')+'_output/'
    ...
    eps_ = []
    batch_ct = 0
    batch = [0.762,0.150,1.291,1.631,1.626]
    for epoch in range(0, config.epochs ):
        ...
        for filename in files_list_:
            ...
            for i, (shower_data,incident_energies) in enumerate(shower_loader_train,0):
                ...
                if isinstance(shower_data, torch.Tensor):
                    print(f'Tensor shape: {shower_data.shape}')
                elif isinstance(shower_data, np.ndarray):
                    print(f'Numpy array shape: {shower_data.shape}')
                elif isinstance(shower_data, list):
                    # Check if it's a list of lists (i.e., multi-dimensional list)
                    if all(isinstance(i, list) for i in shower_data):
                        print(f'List of lists shape: ({len(shower_data)}, {len(shower_data[0])})')
                    else:
                        print(f'List shape: ({len(shower_data)})')
                else:
                    print('shower_data is of an unknown type')
...
def generate(files_list_, load_filename, device='cpu', serialized_model=False):
    ...
    sample_savename = '/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/sampling_result/sampling_20240907_0621_output/sample.pt'
...
#                trained_model_name = 'training_20240408_1350_output/ckpt_tmp_299.pth'
        ...
        if switches_>>3 & trigger:
            ...
            plot_file_name = '/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/sampling_result/sampling_20240907_0621_output/sample.pt'
...
#             Converter_ = Convertor.Convertor_ref(files_list_, 0.0, preprocessor=args.preprocessor)
#             Converter_.invert(-99)
#             Converter_.digitize()
